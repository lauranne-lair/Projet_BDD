...
-- LOT
/*CREATE OR REPLACE PROCEDURE PEUPLE_LOT(nb_lignes IN NUMBER) DETERMINISTIC AS
    id_fournisseur FOURNISSEUR.ID_FOURNISSEUR%TYPE;
BEGIN
    FOR i IN 1..nb_lignes LOOP
        SELECT ID_FOURNISSEUR INTO id_fournisseur FROM (SELECT ID_FOURNISSEUR FROM YOUR_SCHEMA.FOURNISSEUR ORDER BY DBMS_RANDOM.VALUE) WHERE ROWNUM = 1;
        
        INSERT INTO LOT(ID_LOT, ID_FOURNISSEUR, DATE_LIVRAISON_LOT, NB_PLAQUE)
        VALUES(SEQ_ID_LOT.NEXTVAL, id_fournisseur, SYSDATE - TRUNC(DBMS_RANDOM.VALUE(1, 365)), ROUND(DBMS_RANDOM.VALUE(100, 500)));
    END LOOP;
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
...
INSERT INTO APPAREIL (ID_APPAREIL, ID_LISTE, ETAT_APPAREIL, POSITION_APPAREIL) VALUES (1, 1, 'Disponible', 1);
