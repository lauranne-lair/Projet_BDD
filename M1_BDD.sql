...
/*==============================================================*/
/* AJOUT DES CONTRAINT S                               */
/*==============================================================*/
ALTER TABLE LOT ADD CONSTRAINT CK_NB_PLAQUE_LOT CHECK (NB_PLAQUE_LOT = 80);

ALTER TABLE PLAQUE ADD CONSTRAINT CK_NB_SLOT_PLAQUE CHECK (NB_EXPERIENCE_PLAQUE IN (96,384));

ALTER TABLE STOCK ADD CONSTRAINT CK_LIMITE_STOCK_PLAQUE CHECK ((QUANTITE_P384 + QUANTITE_P96) >= 
    (SELECT VOL_DERNIER_TRI_P384 + VOL_DERNIER_TRI_P96 FROM STOCK ORDER BY DATE_TRI DESC FETCH FIRST ROW ONLY)
);

ALTER TABLE SLOT ADD CONSTRAINT Unite_triplet_slot CHECK (
    (RM_SLOT BETWEEN 0 AND 9999) AND
    (RD_SLOT BETWEEN 0 AND 9999) AND
    (VM_SLOT BETWEEN 0 AND 9999) AND
    (VD_SLOT BETWEEN 0 AND 9999) AND
    (BM_SLOT BETWEEN 0 AND 9999) AND
    (BD_SLOT BETWEEN 0 AND 9999) AND
    (TM_SLOT BETWEEN 0 AND 9999) AND
    (TD_SLOT BETWEEN 0 AND 9999)
);

ALTER TABLE EXPERIENCE ADD CONSTRAINT CK_INDICE_PRIORITE_EXPERIENCE CHECK (INDICE_PRIORITE_EXPERIENCE BETWEEN 1 AND 5);
